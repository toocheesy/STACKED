...
## TOAST CSS (add to `css/styles.css`)
...
Scale font sizes proportionally in existing mobile breakpoints if needed (smaller on mobile).
...
**Important:** The points toast and combo toast can fire together on the same capture. The stacking logic handles this — they should appear simultaneously, one above the other.

**Milestone toasts should only fire once per game** — track which milestones have been shown (e.g., a simple object like `{ halfway: false, almost: false }`) and reset it when a new game starts.

---

## SCRIPT TAG in `game.html`

Add `<script src="js/toastManager.js"></script>` in the script loading order — it should load **BEFORE** `main.js` but **AFTER** `ui.js`.

---

## POSITIONING REQUIREMENT

Make sure `.game-area` has `position: relative` if it doesn't already. The toasts use `position: absolute` inside it.

---

## DO NOT TOUCH
- Bot turn logic — no toasts for bots, ever
- Game scoring logic — read the values, don't change how scoring works
- The combo builder or card interactions
- Any other UI systems
- Layout or styling of existing elements

## TEST
1. Play a game, make a capture → see "+X pts" float up from center of play area and fade
2. Make a multi-card combo → see both "+X pts" and "Nice combo!" stacked, appearing simultaneously
3. Use 3+ combo slots → see "HUGE combo!" instead of "Nice combo!"
4. Score past 150 → see "Halfway there!" once (and only once per game)
5. Score past 250 → see "Almost there!" once
6. Bot captures → NO toasts appear
7. Start a new game → milestone tracking resets
