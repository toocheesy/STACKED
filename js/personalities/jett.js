/*
 * Jett "The Blitz" - Advanced Personality
 * Clone of Rex's strategy. Fast, impatient, competitive.
...
const Jett = {
  ...
  evaluateCaptures(hand, board, gameState, mode) {
    ...
    for (const handCard of hand) {
      ...
      const denialValue = Jett._calculateMultiAreaDenial(allCapturedIndices, board);
      const chainPotential = Jett._calculateMultiAreaChain(handCard, allCapturedIndices, hand, board);
      const clearsBoard = allCapturedIndices.size === board.length;
      const compositeScore = Jett._compositeScore(combo.totalPoints, denialValue, chainPotential, clearsBoard, mode);
    ...
    for (const handCard of hand) {
      ...
      for (const capture of captures) {
        ...
        const denialValue = Jett._calculateDenial(capture, hand, board);
        const chainPotential = Jett._calculateChainPotential(handCard, capture, hand, board);
        ...
        allCaptures.push({
          ...
          chainPotential,
          clearsBoard,
          compositeScore: Jett._compositeScore(points, denialValue, chainPotential, clearsBoard, mode)
        });
      }
    }
    ...
    if (bestMultiArea && allCaptures.length > 0) {
      if (mode === 'conservative' && bestMultiArea.points < 10 && bestMultiArea.denialValue < 15) {
        ...
        return Jett._buildMultiAreaMove(bestMultiArea.combo);
      }
  ...
  choosePlacement(hand, board, gameState) {
    ...
    const placements = hand.map(card => {
      ...
      const setupBonus = Jett._calculateSetupValue(card, hand, board);
      score += setupBonus;
